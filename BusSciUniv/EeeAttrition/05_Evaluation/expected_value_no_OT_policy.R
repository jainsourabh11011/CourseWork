...
automl_leader <- h2o.loadModel("04_Modeling/h2o_models/StackedEnsemble_AllModels_0_AutoML_20180710_095314")
...
#  Convert Overtime Yes to No

test_without_OT_tbl <- test_tbl %>% mutate(OverTime = fct_recode(OverTime, "No" = "Yes"))

#  Need to create new predictions
#  Select inital and new state below in the tibble

predictions_without_OT_tbl <- automl_leader %>% h2o.predict(newdata = as.h2o(test_without_OT_tbl)) %>% as.tibble() %>% 
     bind_cols(
          test_tbl %>% 
               select(EmployeeNumber, MonthlyIncome, OverTime), 
          test_without_OT_tbl %>% 
...
#  Note the 5th record is no longer predicted to leave

# Calculate expected value witout OT
