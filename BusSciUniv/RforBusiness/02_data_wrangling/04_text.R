...
# Tibble - creates flags in this case
bikes_tbl %>%
     select(model) %>%
...
# Vector
c("Road - Elite Road - Carbon", "Road - Elite Road") %>% str_split(pattern = " - ", simplify = TRUE) # simplify = TRUE returns a matrix

# separate is a bit easier
...
# 1.6 Formatting Numbers with scales ----
...
bike_orderlines_colnames_tbl <- bike_orderlines_tbl %>%
     rename_at(.vars = vars(model:frame_material), 
               .funs = ~ str_c("prod_", .)) %>% # anonymous function
     rename_at(vars(bikeshop_name:state),
               ~ str_c("cust_", .)) 

glimpse(bike_orderlines_colnames_tbl)
