...
smplBlgs <- getSmpl(f_en_us_blogs, 1)
rm(f_en_us_blogs)
...
# Randomly sample the data sources
smplTwt <- getSmpl(f_en_us_twitter, .5)
rm(f_en_us_twitter)

# Read news text file
f_en_us_news <- readLines(f_en_us_news_name, n = -1L, ok = TRUE, warn = FALSE, encoding = "UTF-8", skipNul = FALSE)
# Randomly sample the data sources
smplNews <- getSmpl(f_en_us_news, 1)
rm(f_en_us_news)

# Combine the three data sets
combData <-  c(smplTwt, smplNews, smplBlgs)

rm(smplTwt, smplNews, smplBlgs, f_en_us_news_name, f_en_us_twitter_name, f_en_us_blogs_name)
...
nComb <- signif(length(combData), 3)
sz = 1200
...
for (i in 1:nDiv ) {
     ...
     print(paste("First loop:", i, "subset", j, k, sep=" "))
}

#rm (fCorpus, freq1, tdm1, freq2, tdm2, freq3, tdm3, freq4, tdm4)

fDF_final <- "./data/fDF1_final"
fName = "./data/fDF1_final.RData"
fDF_name <- "./data/DF1_"

fDF_final <- "./data/fDF2_final"
fName = "./data/fDF2_final.RData"
fDF_name <- "./data/DF2_"

fDF_final <- "./data/fDF3_final"
fName = "./data/fDF3_final.RData"
fDF_name <- "./data/DF3_"

fDF_final <- "./data/fDF4_final"
fName = "./data/fDF4_final.RData"
fDF_name <- "./data/DF4_"
...
fDF1_name <- "./data/fDF1_final"
fDF2_name <- "./data/fDF2_final"
fDF3_name <- "./data/fDF3_final"
fDF4_name <- "./data/fDF4_final"
